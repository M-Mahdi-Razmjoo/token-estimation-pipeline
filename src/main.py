...
from linear_boost_trainer import train_linear_boost_model_pipeline


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--tokenizer', required=True, choices=config.TOKENIZER_COLUMNS.keys())
    parser.add_argument('--model-type', required=True, choices=['linear', 'linear_boost', 'mlp', 'rf', 'et'])
    ...
    elif args.model_type == 'linear_boost':
        train_linear_boost_model_pipeline(
            target_column=target_column,
            language_scope=args.scope,
            tokenizer_name=args.tokenizer
        )
